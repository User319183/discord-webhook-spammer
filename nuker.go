...
func spam(webhook string, msg string, tts bool, sleep time.Duration, wg *sync.WaitGroup, success *int, fail *int, errors *int) {
    defer wg.Done()
    for {
        proxy := getProxy()
...
func main() {
    ...
    for i := 0; i < threads; i++ {
        for _, webhook := range webhooks {
            wg.Add(1)
            go spam(webhook, strings.TrimSpace(msg), tts, time.Duration(sleep)*time.Second, &wg, &success, &fail, &errors)
        }
    }
    wg.Wait()
}
